...
from kimodo.runtime import choose_torch_device
...
class LLM2VecEncoder:
    ...
    def __init__(
        ...
        pre_peft_model_name_or_path = os.environ.get("TEXT_ENCODER_MNTP_ADAPTER_PATH")
        if pre_peft_model_name_or_path and "TEXT_ENCODERS_DIR" in os.environ and not os.path.isabs(pre_peft_model_name_or_path):
            pre_peft_model_name_or_path = os.path.join(os.environ["TEXT_ENCODERS_DIR"], pre_peft_model_name_or_path)

        self.model = LLM2Vec.from_pretrained(
            base_model_name_or_path=base_model_name_or_path,
            peft_model_name_or_path=peft_model_name_or_path,
            pre_peft_model_name_or_path=pre_peft_model_name_or_path,
        ...
        if env_device:
            device = env_device
        device = choose_torch_device(default=device, env_var="TEXT_ENCODER_DEVICE")
        self._device = device
    ...
    def __call__(self, text: list[str] | str):
        ...
        if isinstance(encoded_text, torch.Tensor):
            encoded_text = encoded_text.detach().clone().to(self._device)
        else:
