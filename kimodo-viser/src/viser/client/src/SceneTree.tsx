        ...
        makeObject: (ref, children) => (
          <group onClick={(e) => e.stopPropagation()}>
            <PivotControls
              ...
              onDragStart={() => {
                isDragging = true;
                viewer.mutable.current.transformControlsDraggingNames.add(
                  message.name,
                );
              ...
              onDragEnd={() => {
                if (isDragging) {
                  isDragging = false;
                  viewer.mutable.current.transformControlsDraggingNames.delete(
                    message.name,
                  );
                  flushDragMessage();
                  viewer.mutable.current.sendMessage({
...
export function SceneNodeThreeObject(props: { name: string }) {
  ...
  const highlightGroup =
    viewer.useSceneTree((state) => state[props.name]?.highlight_group) ?? null;
  ...
  useFrame(
    () => {
      ...
      if (node.poseUpdateState == "needsUpdate") {
        // Skip applying pose from scene state while the user is dragging this
        // transform control; PivotControls owns the ref's matrix during drag.
        // Applying server or local state here would cause jitter.
        const isTransformControlsDragging =
          message?.type === "TransformControlsMessage" &&
          viewer.mutable.current.transformControlsDraggingNames.has(
            props.name,
          );
        if (!isTransformControlsDragging) {
          updateNodeAttributes(props.name, {
          ...
          if (!objRef.current.matrixAutoUpdate)
  ...
  if (!clickable && hoveredRef.current.isHovered) {
    ...
    if (highlightGroup) {
      viewerMutable.hoveredHighlightGroups.delete(highlightGroup);
    }
      ...
      <>
          ...
          onPointerOver={
            !clickable
              ...
              : (e) => {
                  ...
                  if (highlightGroup) {
                    viewerMutable.hoveredHighlightGroups.add(highlightGroup);
                  }
        ...
        >
          <HoverableContext.Provider
            value={{ state: hoveredRef, clickable, highlightGroup: highlightGroup ?? undefined }}
          >
            {objNode}
          </HoverableContext.Provider>
        </group>
