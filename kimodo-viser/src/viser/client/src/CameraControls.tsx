...
export function SynchronizedCameraControls() {
  ...
  React.useEffect(() => {
    ...
    // Check if arrow keys should be enabled for camera control
    ...
    const keys = {
      ...
      // Only create arrow key listeners if camera keyboard controls are enabled
      ...(enableArrowKeys ? {
  ...
  }, [CameraControls, viewer.cameraKeyboardControlsEnabled.value]);
