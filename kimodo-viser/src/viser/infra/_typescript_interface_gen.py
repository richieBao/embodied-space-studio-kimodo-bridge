...
def _get_ts_type(typ: Type[Any]) -> str:
    ...
    if origin_typ is tuple:
        ...
        if dataclasses.is_dataclass(typ):
            ...
            # Include NotRequired keys from __annotations__ (get_type_hints omits them).
            annotations = getattr(typ, "__annotations__", {})
            for key in annotations:
                if key not in hints:
                    val = annotations[key]
                    if get_origin(val) is NotRequired:
                        hints[key] = get_args(val)[0]
        optional_keys = getattr(typ, "__optional_keys__", set())
        if not isinstance(optional_keys, set):
