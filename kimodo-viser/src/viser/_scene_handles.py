...
@dataclasses.dataclass(frozen=True)
class KeyboardEvent:
    """Event passed to keyboard callbacks."""

    client: ClientHandle
    """Client that triggered this event."""
    client_id: int
    """ID of client that triggered this event."""
    event_type: _messages.KeyboardEventType
    """Type of event that was triggered (keydown or keyup)."""
    key: str
...
class SceneNodeHandle(AssignablePropsBase[_SceneNodeHandleState]):
    ...
    def sync_wxyz(self, wxyz: tuple[float, float, float, float] | np.ndarray) -> None:
        """Set orientation and always send to clients (skip allclose check).
        Use when the client must receive the value even if it is unchanged (e.g. after drag end).
        """
        from ._scene_api import cast_vector
    ...
    def sync_position(
        self, position: tuple[float, float, float] | np.ndarray
    ) -> None:
        """Set position and always send to clients (skip allclose check).
        Use when the client must receive the value even if it is unchanged (e.g. after drag end).
        """
        from ._scene_api import cast_vector
...
class _ClickableSceneNodeHandle(SceneNodeHandle):
    def on_click(
        ...
        *,
        ...
        Args:
            func: Callback to run when the node is clicked.
            highlight_group: If set, when this node is hovered, all nodes
                with the same highlight_group will show the hover highlight
                (e.g. so a multi-mesh character highlights as one).
        """
        self._impl.api._websock_interface.queue_message(
            _messages.SetSceneNodeClickableMessage(
                self._impl.name, True, highlight_group
            )
        )
