...
class SceneApi:
    ...
    def __init__(
        ...
        self._keyboard_cb: (
            Callable[[KeyboardEvent], None | Coroutine] | None
        ) = None
        self._keyboard_done_cb: Callable[[], None | Coroutine] = lambda: None
        self._keyboard_debounce_ms: float | None = None
        self._keyboard_last_trigger_time: dict[str, float] = {}
        
        # Keyboard repeat system (like text editors)
        self._keyboard_repeat_delay_ms: float | None = None
        self._keyboard_repeat_interval_ms: float | None = None
        self._keyboard_repeat_state: dict[str, dict[str, float | bool]] = {}  # key -> {first_press_time, last_trigger_time, repeating}
        ...
        self._websock_interface.register_handler(
            _messages.KeyboardMessage,
            self._handle_keyboard_updates,
        )
    ...
    def add_transform_controls(
        ...
        *,
        ...
        Args:
            ...
            space: Gizmo axes in 'world' (scene) or 'local' (object) space.
            wxyz: Quaternion rotation to parent frame from local frame (R_pl).
            position: Translation from parent frame to local frame (t_pl).
    ...
    async def _handle_keyboard_updates(
        self, client_id: ClientId, message: _messages.KeyboardMessage
    ) -> None:
        """Callback for handling keyboard messages."""
        # Call the callback if it exists.
        if self._keyboard_cb is None:
            return
        ...
        # Check if keyboard repeat is enabled (new system)
        if self._keyboard_repeat_delay_ms is not None and self._keyboard_repeat_interval_ms is not None:
            if message.event_type == "keydown":
                # Initialize state for this key if it's the first press
                if key not in self._keyboard_repeat_state:
                    self._keyboard_repeat_state[key] = {
                        "first_press_time": current_time,
                        "last_trigger_time": current_time,
                        "repeating": False,
                    }
                    should_trigger = True  # Always trigger on first press
                else:
                    # Key is being held down
                    state = self._keyboard_repeat_state[key]
                    time_since_first = (current_time - state["first_press_time"]) * 1000
                    time_since_last = (current_time - state["last_trigger_time"]) * 1000
                    
                    if not state["repeating"]:
                        # Check if we've passed the initial delay
                        if time_since_first >= self._keyboard_repeat_delay_ms:
                            state["repeating"] = True
                            state["last_trigger_time"] = current_time
                            should_trigger = True
                    else:
                        # We're in repeat mode, check if enough time has passed for next repeat
                        if time_since_last >= self._keyboard_repeat_interval_ms:
                            state["last_trigger_time"] = current_time
                            should_trigger = True
            elif message.event_type == "keyup":
                # Clear the state when key is released
                if key in self._keyboard_repeat_state:
                    del self._keyboard_repeat_state[key]
                should_trigger = True  # Always trigger keyup events
        
        # Check if old debouncing is enabled (legacy system)
        elif self._keyboard_debounce_ms is not None:
            # Enhanced debounce logic: track keydown/keyup state per key
            # This prevents same keydown from triggering multiple times during a single press-hold cycle
            if message.event_type == "keydown":
                # Check if this key is already in a pressed state (held down)
                if key in self._keyboard_repeat_state:
                    # Key is being held - this is a repeat keydown from OS
                    # Check debounce window
                    if key in self._keyboard_last_trigger_time:
                        time_since_last = (current_time - self._keyboard_last_trigger_time[key]) * 1000
                        if time_since_last < self._keyboard_debounce_ms:
                            # Within debounce window - ignore
                            return
                    # Past debounce window - allow (OS repeat)
                    self._keyboard_last_trigger_time[key] = current_time
                    should_trigger = True
                else:
                    # First press of this key
                    self._keyboard_repeat_state[key] = {"pressed": True}
                    self._keyboard_last_trigger_time[key] = current_time
                    should_trigger = True
            
            elif message.event_type == "keyup":
                # Key released - clear the pressed state
                if key in self._keyboard_repeat_state:
                    del self._keyboard_repeat_state[key]
                # Trigger callback for keyup events too so user can handle key release
                should_trigger = True
        
        else:
            # No debouncing or repeat system - trigger every event
            should_trigger = True
        
        # Only trigger callback if we should
        ...
        if asyncio.iscoroutinefunction(self._keyboard_cb):
            await self._keyboard_cb(event)
        else:
            self._thread_executor.submit(
                self._keyboard_cb, event
            ).add_done_callback(print_threadpool_errors)
    ...
    def on_keyboard_event(
        ...
        repeat_delay_ms: float | None = None,
        repeat_interval_ms: float | None = None,
    ) -> Callable[
        [Callable[[KeyboardEvent], None]], Callable[[KeyboardEvent], None]
    ]:
        """Add a callback for keyboard events.

        Args:
            event_type: Type of keyboard event to listen for ('keydown' or 'keyup').
            debounce_ms: Optional debounce time in milliseconds. If specified, the same key
                pressed within this time window will be ignored. Useful for preventing
                rapid repeated key presses (e.g., 200ms for play/pause controls).
                Note: This is the legacy system. Use repeat_delay_ms and repeat_interval_ms
                for better control (like text editors).
            repeat_delay_ms: Optional initial delay in milliseconds before key repeat starts
                when holding down a key (e.g., 500ms). Must be used with repeat_interval_ms.
                This provides text-editor-like behavior: press and hold a key, it triggers once,
                waits for repeat_delay_ms, then repeats every repeat_interval_ms.
            repeat_interval_ms: Optional interval in milliseconds between repeats after the
                initial delay (e.g., 50ms). Must be used with repeat_delay_ms.

        Returns:
            Decorator function for registering a callback.
        ...
        Example:
            ...
            >>> def handle_key(event: viser.KeyboardEvent):
            >>>     if event.key == " ":  # Space key
            >>>         print("Space pressed!")
            >>> 
            >>> # Legacy debouncing (simple time window)
            >>> @server.scene.on_keyboard_event("keyup", debounce_ms=200)
            >>> def handle_key_debounced(event: viser.KeyboardEvent):
            >>>     if event.key == " ":
            >>>         print("Space pressed (debounced)!")
            >>>
            >>> # New repeat system (text-editor-like behavior)
            >>> @server.scene.on_keyboard_event("keydown", repeat_delay_ms=500, repeat_interval_ms=50)
            >>> def handle_arrow_key(event: viser.KeyboardEvent):
            >>>     if event.key == "ArrowRight":
            >>>         print("Moving right...")
            >>>         # Pressing and holding: triggers once, waits 500ms, then repeats every 50ms
        """
        # Ensure the event type is valid.
        assert event_type in get_args(_messages.KeyboardEventType)
        
        # Validate repeat parameters
        if (repeat_delay_ms is not None or repeat_interval_ms is not None):
            if repeat_delay_ms is None or repeat_interval_ms is None:
                raise ValueError(
                    "Both repeat_delay_ms and repeat_interval_ms must be specified together."
                )
            if debounce_ms is not None:
                raise ValueError(
                    "Cannot use both debounce_ms and repeat system (repeat_delay_ms/repeat_interval_ms). "
                    "Choose one approach."
                )
        ...
        def cleanup_previous_event(target: ViserServer | ClientHandle):
            # If the server or client does not have a keyboard callback, return.
            if target.scene._keyboard_cb is None:
                return

            # Remove callback.
            target.scene.remove_keyboard_callback()

        def decorator(
            func: Callable[[KeyboardEvent], None],
        ) -> Callable[[KeyboardEvent], None]:
            # Check if another keyboard event was previously registered.
            # If so, we need to clear the previous event and register the new one.
            cleanup_previous_event(self._owner)
            ...
            self._keyboard_cb = func
            self._keyboard_debounce_ms = debounce_ms
            self._keyboard_repeat_delay_ms = repeat_delay_ms
            self._keyboard_repeat_interval_ms = repeat_interval_ms

            self._websock_interface.queue_message(
                _messages.KeyboardListenMessage(enable=True)
            )
            return func

        return decorator

    def on_keyboard_callback_removed(
        self,
        func: Callable[[], NoneOrCoroutine],
    ) -> Callable[[], NoneOrCoroutine]:
        """Add a callback to run automatically when the callback for a keyboard
        event is removed. This will be triggered exactly once, either
        manually (via :meth:`remove_keyboard_callback()`) or automatically (if
        the keyboard event is overridden with another call to
        :meth:`on_keyboard_event()`).

        Args:
            func: Callback for when keyboard events are removed.
        """
        self._keyboard_done_cb = func
        return func

    def remove_keyboard_callback(
        self,
    ) -> None:
        """Remove the currently attached keyboard event callback. This will trigger
        any callback attached to `.on_keyboard_callback_removed()`."""

        if self._keyboard_cb is None:
            warnings.warn(
                "No keyboard callback exists for this server/client, ignoring.",
                stacklevel=2,
            )
            return
        ...
        # Notify client that the listener has been removed.
        self._websock_interface.queue_message(
            _messages.KeyboardListenMessage(enable=False)
        )
        self._owner.flush()

        # Run cleanup callback.
        if asyncio.iscoroutinefunction(self._keyboard_done_cb):
            self._event_loop.create_task(self._keyboard_done_cb())
        else:
            self._keyboard_done_cb()

        # Reset the callback on the python side.
        self._keyboard_cb = None
        self._keyboard_done_cb = lambda: None
        self._keyboard_debounce_ms = None
        self._keyboard_last_trigger_time.clear()
        self._keyboard_repeat_delay_ms = None
        self._keyboard_repeat_interval_ms = None
        self._keyboard_repeat_state.clear()
