...
from ._timeline_api import TimelineApi
...
class CameraHandle:
    ...
    def _update_wxyz(self) -> None:
        ...
        if z_norm < 1e-8:
            # Avoid division by zero when look_at equals position.
        ...
        if y_norm < 1e-8:
            # Fall back to a stable orthogonal vector if up is parallel to z.
            fallback = np.array([0.0, 1.0, 0.0], dtype=np.float64)
            if abs(np.dot(fallback, z)) > 0.99:
                fallback = np.array([1.0, 0.0, 0.0], dtype=np.float64)
            y = fallback - np.dot(z, fallback) * z
            y /= np.linalg.norm(y)
        else:
            y /= y_norm
        x = np.cross(y, z)
...
class ClientHandle(DeprecatedAttributeShim if not TYPE_CHECKING else object):
    ...
    def __init__(
        ...
        self.timeline: TimelineApi = TimelineApi(
            self, 
            websock_interface=conn,
            thread_executor=server._thread_executor,
            event_loop=server._event_loop
        )
        """Handle for interacting with the timeline UI."""
...
class ViserServer(DeprecatedAttributeShim if not TYPE_CHECKING else object):
    ...
    **Shared state.** Elements added to the server object, for example via a
    ...
    Args:
        ...
        enable_camera_keyboard_controls: If True (default), WASD and arrow keys 
            control the camera. If False, arrow keys are available for custom 
            keyboard event handlers (WASD still controls camera).
        show_timeline_arrow_keys: If True, shows a semi-transparent arrow-key overlay
            (↑ ↓ ← →) above the timeline. Position and highlighting can be configured
    ...
    def __init__(
        ...
        label: str | None = None,
        verbose: bool = True,
        enable_camera_keyboard_controls: bool = True,
        show_timeline_arrow_keys: bool = False,
        **_deprecated_kwargs,
    ):
        ...
        self.timeline: TimelineApi = TimelineApi(
            self, 
            websock_interface=server,
            thread_executor=self._thread_executor,
            event_loop=self._event_loop
        )
        """Handle for interacting with the timeline UI."""

        # Configure optional UI-only arrow-key overlay above the timeline.
        self.timeline.configure_arrow_key_overlay(enabled=show_timeline_arrow_keys)
        ...
        # Send camera keyboard controls setting
        server.queue_message(
            _messages.CameraKeyboardControlsMessage(enable=enable_camera_keyboard_controls)
        )
    ...
    def configure_arrow_key_overlay(
        ...
        *,
        ...
        self.timeline.configure_arrow_key_overlay(
            enabled=enabled,
